...
#
# A module for moderator commands. By Zemo
# Users require the
...
class Mod:
    ...
    async def mute(self, ctx, user: discord.Member, bugs: str, time: int=1):
        rant = random.randint(1, 255)
        embed = discord.Embed(title=f"{user.name} has been muted!", value=bugs, color=rant)
        role = discord.utils.get(user.server.roles, name="Muted")
        await self.bot.add_roles(user, role)
        await self.bot.send_message(self.bot.get_channel("479080357207998464"), embed=embed)
