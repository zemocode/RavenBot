...
#
# A module of owner commands. by Zemo
...
class Owner:
    ...
    async def _set(self, ctx):
        """Changes raven's global settings."""
    ...
    async def status(self, ctx, *, status=None):
        """Sets ravens's status

        Leaving this empty will clear it."""

        if status:
            status = status.strip()
            await self.bot.change_presence(discord.Game(name=status))
