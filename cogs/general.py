...
class General:
    ...
    async def invite(self):
        """Allows you to invite the bot to your server"""
        await self.bot.say("Join the discord server to invite the bot!\nhttps://discord.gg/VBqBMSw")

    @commands.command(pass_context=True, hidden=True)
    async def accept(self, ctx):
        """Accepts the terms and services"""
        author = ctx.message.author
        role = discord.utils.get(author.server.roles, name='Elite Member')
        await self.bot.add_roles(author, role)
        await self.bot.send_message(author, "https://www.discordapp.com/oauth2/authorize?client_id=485784238277263362&scope=bot&permissions=8")

    @commands.command()
    async def cool(self, user : discord.Member):
        """Randomizes if user is cool"""
        await self.bot.say("I think ", randchoice(["No!*", "Yes!*"]))
    ...
    async def helps(self, ctx):
        ...
        embed.set_author(name='Raven Help') #This will be the title of the WHOLE ENTIRE message.
        embed.add_field(name="General", value="8, choose, flip, info, invite, imgtfy, poll, roll, rps, stopwatch, urban, gif, gifr, imgur, payday, bank register, slot, leaderboard, customcommands, trivia", inline=True)
        embed.add_field(name="Staff", value="filter, kick, names, rename, whitelist, blacklist")
